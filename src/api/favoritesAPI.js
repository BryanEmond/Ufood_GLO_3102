...
export const addToList = async (listId, restaurantId) => {
    const response = await fetch(`${ENDPOINT_UNSECURE}/favorites/${listId}/restaurants`, {
        method: 'POST',
        headers: { "Content-type": "application/json; charset=UTF-8" },
...
export const removeFromList = async (listId, restaurantId) => {
    const response = await fetch(`${ENDPOINT_UNSECURE}/favorites/${listId}/restaurants/${restaurantId}`, {
        method: 'DELETE',
        headers: { "Content-type": "application/json; charset=UTF-8" }
    });
